...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) {
        // Initialize Database and Verification Server in Background (Non-blocking startup)
        new Thread(() -> {
            com.skilora.config.DatabaseInitializer.initialize();
            com.skilora.service.formation.VerificationServer.start();
        }).start();

        // Register shutdown hook to close database connections and stop servers
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            try {
                com.skilora.service.formation.VerificationServer.stop();
