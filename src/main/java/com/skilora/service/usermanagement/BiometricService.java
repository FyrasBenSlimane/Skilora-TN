...
import com.skilora.model.entity.usermanagement.BiometricData;
...
public class BiometricService {
    ...
    private JSONObject runPythonService(String command, String username, BufferedImage image) {
        try {
            ...
            ProcessBuilder pb;
            if (username != null) {
                pb = new ProcessBuilder("python", PYTHON_SCRIPT_PATH, command, username);
            } else {
                pb = new ProcessBuilder("python", PYTHON_SCRIPT_PATH, command);
            }

            pb.directory(new File(".")); // Run in project root
            Process process = pb.start();

            // Send Image Data to Stdin and close it properly
            try (BufferedWriter writer = new BufferedWriter(
                ...
                logger.warn("Error writing to Python process stdin (process may have closed): {}", e.getMessage());
                // Continue to read output even if stdin write failed
            }
            ...
            try (BufferedReader reader = new BufferedReader(
                ...
                logger.warn("Error reading from Python process stdout: {}", e.getMessage());
            }
        ...
        try (Connection conn = DatabaseConfig.getInstance().getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    if (ts != null) data.setRegisteredAt(ts.getTime());
     ...
     * The Python service will reject duplicates (same face already linked to another account).
     *
     ...
     * @param excludeUsername  username to exclude from duplicate check (e.g. current user re-registering)
