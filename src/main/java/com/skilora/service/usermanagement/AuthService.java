...
public class AuthService {
    ...
    public Optional<User> login(String username, String password) {
        try {
            // Check rate limiting - catch any issues to prevent login hang
            try {
                ...
                logger.error("Error checking lockout status, bypassing", e);
            }

            Optional<User> userOpt = com.skilora.service.usermanagement.UserService.getInstance().findByUsername(username);
            if (userOpt.isPresent()) {
                User user = userOpt.get();
                if (com.skilora.service.usermanagement.UserService.verifyPassword(password, user.getPassword())) {
                    // Start a separate thread to record the attempt so it doesn't block the login response
                    new Thread(() -> {
                        try {
                            recordLoginAttempt(username, true);
                        } catch (Exception e) {
                            logger.error("Failed to record success attempt", e);
                        }
                    }).start();
            ...
            new Thread(() -> {
                try {
                    ...
                    logger.error("Failed to record failure attempt", e);
                }
            }).start();
            
        } catch (Exception e) {
            logger.error("Critical error in login service", e);
        }
        return Optional.empty();
    }
     ...
     */
    public Optional<User> loginWithBiometric(String username) {
        Optional<User> userOpt = com.skilora.service.usermanagement.UserService.getInstance().findByUsername(username);
        if (userOpt.isPresent() && com.skilora.service.usermanagement.BiometricService.getInstance().hasBiometricData(username)) {
            return userOpt;
        }
        return Optional.empty();
    }
    ...
    public Optional<User> getUser(String username) {
        return com.skilora.service.usermanagement.UserService.getInstance().findByUsername(username);
    }
    ...
    public void register(User user) {
        if (com.skilora.service.usermanagement.UserService.getInstance().findByUsername(user.getUsername()).isPresent()) {
            throw new RuntimeException("Username already exists");
        }
        com.skilora.service.usermanagement.UserService.getInstance().create(user);
    }
    ...
    /**
     * Reset lockout for a user by clearing their failed login attempts.
     * Useful for development/testing or admin unlock.
     */
    public void resetLockout(String username) {
        ...
        try (Connection conn = DatabaseConfig.getInstance().getConnection();
            ...
            if (deleted > 0) {
                logger.info("Reset lockout for user: {} (deleted {} failed attempts)", username, deleted);
            }
        } catch (SQLException e) {
            logger.error("Could not reset lockout for user: {}", username, e);
        }
    }
}
