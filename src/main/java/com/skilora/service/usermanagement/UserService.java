...
public class UserService {
    ...
    public void create(User user) {
        String query = "INSERT INTO users (username, email, password, role, photo_url, is_verified, is_active) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (Connection connection = DatabaseConfig.getInstance().getConnection();
    ...
    public void update(User user) {
        String query = "UPDATE users SET username = ?, email = ?, password = ?, role = ?, " +
                "photo_url = ?, is_verified = ?, is_active = ? WHERE id = ?";
        try (Connection connection = DatabaseConfig.getInstance().getConnection();
                PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setString(1, user.getUsername());
            stmt.setString(2, user.getEmail());
    ...
    private User mapResultSetToUser(ResultSet rs) throws SQLException {
        ...
        try {
            ...
            // Handle legacy role values or mismatches
            if ("JOB_SEEKER".equals(roleStr)) {
                role = Role.JOB_SEEKER;
            } else if ("USER".equals(roleStr)) {
                role = Role.USER;
            } else {
                logger.warn("Unknown role '{}', defaulting to USER", roleStr);
                role = Role.USER;
            }
        }
