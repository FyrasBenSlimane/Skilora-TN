...
import com.skilora.service.usermanagement.BiometricService;
import com.skilora.model.service.FastCameraManager;
...
public class BiometricAuthController implements Initializable {
    ...
    public static void showDialog(Stage owner, String username, Consumer<String> onSuccess) {
        try {
            FXMLLoader loader = new FXMLLoader(
                    BiometricAuthController.class.getResource("/com/skilora/view/BiometricAuthDialog.fxml"));
            ...
            controller.titleLabel.setText(username != null ? "Vérification Biométrique" : "Identification Biométrique");
    ...
    public static void showRegistrationDialog(Stage owner, String username, Consumer<String> onSuccess) {
        try {
            ...
            dialogStage.initStyle(StageStyle.TRANSPARENT);
            controller.dialogStage = dialogStage;
            controller.titleLabel.setText("Enregistrement Biométrique");
    ...
    private void startCamera() {
        Thread cameraThread = new Thread(() -> {
            ...
            if (webcam != null) {
                ...
                Platform.runLater(() -> statusLabel.setText("Caméra non disponible"));
            }
        }, "CameraStartThread");
    ...
    private void runRegistrationLoop() {
            ...
            if (webcam != null && webcam.isOpen()) {
                ...
                if (bimg != null) {
                    ...
                    if (result != null && result.optBoolean("success", false)) {
                        ...
                        String matchedUser = result.optString("matched_username", "un autre compte");
                        Platform.runLater(() -> {
                            statusLabel.setText("⚠ Ce visage est déjà enregistré pour le compte '" + matchedUser + "'");
                        ...
                        String msg = result != null ? result.optString("message", "Aucun visage détecté") : "Aucun visage détecté";
        ...
        if (isRunning.get()) {
            Platform.runLater(() -> {
                statusLabel.setText("Délai expiré. Aucun visage détecté.");
    ...
    private void runVerificationLoop() {
        ...
        Platform.runLater(() -> statusLabel.setText(
                username != null ? "Vérification en cours..." : "Identification en cours..."));
            ...
            if (webcam != null && webcam.isOpen()) {
                ...
                if (bimg != null) {
                    ...
                    if (matchThisFrame) {
                        ...
                        final double progress = (double) consecutiveMatches / CONSENSUS_REQUIRED;
                        Platform.runLater(() -> {
                            updateConfidence(Math.min(conf, 1.0));
                            statusLabel.setText("Visage détecté (" + consecutiveMatches + "/" + CONSENSUS_REQUIRED + ")...");
                        ...
                        if (consecutiveMatches >= CONSENSUS_REQUIRED) {
                            ...
                            Platform.runLater(() -> {
                                statusLabel.setText("✓ Bienvenue, " + finalUser + " !");
                                statusLabel.setStyle("-fx-text-fill: -fx-success; -fx-font-weight: bold;");
                                animateViewfinderSuccess();
                                updateConfidence(1.0);
                            });
                        ...
                        Platform.runLater(() -> {
                            ...
                            statusLabel.setText(username != null ? "Vérification..." : "Recherche...");
                        });
                    }
                }
            }
        ...
        if (isRunning.get()) {
            Platform.runLater(() -> {
                statusLabel.setText("Échec de la vérification.");
                statusLabel.setStyle("-fx-text-fill: -fx-destructive; -fx-font-weight: bold;");
                animateViewfinderError();
            });
            sleepQuiet(2000);
