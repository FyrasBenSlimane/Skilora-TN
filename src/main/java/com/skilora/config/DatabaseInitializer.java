...
public class DatabaseInitializer {
    ...
    private static void ensureUsersTableColumns(Statement stmt) {
        ...
        // Ensure photo_url columns are LONGTEXT (for Base64 data URI storage)
        ensurePhotoUrlLongText(stmt, "users");
        ensurePhotoUrlLongText(stmt, "profiles");

        // Ensure profile extended columns exist (headline, bio, website)
        if (!columnExists(stmt, "profiles", "headline")) {
            try {
                stmt.execute("ALTER TABLE profiles ADD COLUMN headline VARCHAR(255) DEFAULT NULL");
                logger.info("Added 'headline' column to profiles table.");
            } catch (SQLException e) {
                logger.debug("Could not add headline column: {}", e.getMessage());
            }
        }
        if (!columnExists(stmt, "profiles", "bio")) {
            try {
                stmt.execute("ALTER TABLE profiles ADD COLUMN bio TEXT DEFAULT NULL");
                logger.info("Added 'bio' column to profiles table.");
            } catch (SQLException e) {
                logger.debug("Could not add bio column: {}", e.getMessage());
            }
        }
        if (!columnExists(stmt, "profiles", "website")) {
            try {
                stmt.execute("ALTER TABLE profiles ADD COLUMN website VARCHAR(255) DEFAULT NULL");
                logger.info("Added 'website' column to profiles table.");
            } catch (SQLException e) {
                logger.debug("Could not add website column: {}", e.getMessage());
            }
        }
    }

    /**
     * Ensures the photo_url column is LONGTEXT to support Base64-encoded images.
     * If the column exists but is not LONGTEXT, it will be altered.
     */
    private static void ensurePhotoUrlLongText(Statement stmt, String tableName) {
        if (!columnExists(stmt, tableName, "photo_url")) return;
        try {
            // Check current column type
            var rs = stmt.executeQuery(
                    "SELECT DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS " +
                    "WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = '" + tableName + "' AND COLUMN_NAME = 'photo_url'");
            if (rs.next()) {
                String dataType = rs.getString("DATA_TYPE").toLowerCase();
                if (!"longtext".equals(dataType)) {
                    stmt.execute("ALTER TABLE " + tableName + " MODIFY COLUMN photo_url LONGTEXT");
                    logger.info("Migrated {}.photo_url to LONGTEXT for Base64 image storage.", tableName);
                }
            }
            rs.close();
        } catch (SQLException e) {
            logger.debug("Could not check/alter {}.photo_url: {}", tableName, e.getMessage());
        }
    }
