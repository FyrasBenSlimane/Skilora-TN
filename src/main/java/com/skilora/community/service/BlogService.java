...
public class BlogService {
    ...
    public int create(BlogArticle article) {
        ...
        if (article.getTitle() == null || article.getTitle().isBlank()) throw new IllegalArgumentException("Article title is required");
        if (article.getAuthorId() <= 0) throw new IllegalArgumentException("Valid author ID is required");
        String sql = """
            INSERT INTO blog_articles (author_id, title, content, summary, cover_image_url,
                category, tags, is_published, published_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """;
        try (Connection conn = DatabaseConfig.getInstance().getConnection();
            ...
            if (rs.next()) {
                int id = rs.getInt(1);
                logger.info("Blog article created: id={}", id);
                return id;
            }
    ...
    public BlogArticle findById(int id) {
        String sql = SELECT_WITH_AUTHOR + "WHERE b.id = ?";
        try (Connection conn = DatabaseConfig.getInstance().getConnection();
            ...
            logger.error("Error finding article {}: {}", id, e.getMessage(), e);
        }
        return null;
    }
    ...
    public List<BlogArticle> findAll() {
        ...
        String sql = SELECT_WITH_AUTHOR + "ORDER BY b.created_date DESC";
        try (Connection conn = DatabaseConfig.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) articles.add(mapArticle(rs));
        } catch (SQLException e) {
            logger.error("Error fetching all articles: {}", e.getMessage(), e);
        }
        return articles;
    }
    ...
    public List<BlogArticle> findPublished() {
        List<BlogArticle> articles = new ArrayList<>();
        String sql = SELECT_WITH_AUTHOR + "WHERE b.is_published = TRUE ORDER BY b.published_date DESC";
        try (Connection conn = DatabaseConfig.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) articles.add(mapArticle(rs));
    ...
    public List<BlogArticle> findByCategory(String category) {
        List<BlogArticle> articles = new ArrayList<>();
        String sql = SELECT_WITH_AUTHOR + "WHERE b.is_published = TRUE AND b.category = ? ORDER BY b.published_date DESC";
        try (Connection conn = DatabaseConfig.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            logger.error("Error fetching articles by category: {}", e.getMessage(), e);
        }
        return articles;
    }
    ...
    public boolean update(BlogArticle article) {
        String sql = """
            UPDATE blog_articles SET title = ?, content = ?, summary = ?,
                cover_image_url = ?, category = ?, tags = ?,
                is_published = ?, published_date = ?, updated_date = NOW()
            WHERE id = ?
            """;
        try (Connection conn = DatabaseConfig.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public boolean incrementLikes(int id) {
        String sql = "UPDATE blog_articles SET likes_count = likes_count + 1 WHERE id = ?";
        try (Connection conn = DatabaseConfig.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, id);
            return stmt.executeUpdate() > 0;
        } catch (SQLException e) {
            logger.error("Error incrementing likes: {}", e.getMessage(), e);
        }
        return false;
    }
    ...
    public List<BlogArticle> search(String query) {
        ...
        String sql = SELECT_WITH_AUTHOR +
                "WHERE b.is_published = TRUE AND (b.title LIKE ? OR b.summary LIKE ? OR b.tags LIKE ?) " +
