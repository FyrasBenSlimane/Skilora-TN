 ...
 * Credentials are loaded from EnvConfig (.env file).
 */
public class EmailService {
    ...
    private EmailService() {
        ...
        if (envPassword == null || envPassword.isBlank()) {
            logger.warn("SKILORA_MAIL_PASSWORD not set. Email sending will fail. " +
                    "Set SKILORA_MAIL_EMAIL and SKILORA_MAIL_PASSWORD in .env file.");
        } else {
            logger.info("EmailService initialized with email: {}", fromEmail);
        }
    }
    ...
    /**
     * Send a security alert email to admin when login fails.
     */
    public void sendSecurityAlertEmail(String adminEmail, String username, int failedAttempts,
                                       String photoPath, boolean isLockedOut) {
        String subject = isLockedOut ? "ALERTE: Compte verrouille - " + username : "Alerte de securite - " + username;
        String body = buildSecurityAlertBody(username, failedAttempts, photoPath, isLockedOut);
        try {
            sendEmail(adminEmail, subject, body);
            logger.info("Security alert email sent to admin for user: {}", username);
        } catch (Exception e) {
            logger.error("Failed to send security alert email", e);
        }
    }

    private String buildSecurityAlertBody(String username, int failedAttempts, String photoPath, boolean isLockedOut) {
        String status = isLockedOut ? "COMPTE VERROUILLE" : "Alerte de connexion";
        String color = isLockedOut ? "#dc2626" : "#f59e0b";
        return """
                <!DOCTYPE html>
                <html>
                    ...
                    <table width="100%%" cellpadding="0" cellspacing="0">
                        <tr><td align="center" style="padding:40px 0;">
                            <table width="600" cellpadding="0" cellspacing="0" style="background:#fff; border-radius:12px;">
                                <tr><td style="padding:30px 40px; background:%s; color:#fff; border-radius:12px 12px 0 0;">
                                    <h2 style="margin:0;">%s</h2>
                                </td></tr>
                                <tr><td style="padding:40px;">
                                    <p style="font-size:16px; color:#374151;">
                                        <strong>Utilisateur:</strong> %s<br>
                                        <strong>Tentatives echouees:</strong> %d<br>
                ...
                """.formatted(color, status, username, failedAttempts,
                        photoPath != null ? photoPath : "N/A",
