...
/**
 * Validation Helper - Strict rules for all finance fields.
 */
public class ValidationHelper {
    ...
    // Relaxed IBAN Pattern for testing: Min 10 chars
    ...
    public static String validateName(String name) {
        ...
        if (!NAME_PATTERN.matcher(name.trim()).matches()) return "Name must contain only letters!";
        return null;
    }
    ...
    public static String validatePhone(String phone) {
        ...
        if (!cleaned.matches("^[0-9]+$")) return "Phone must contain only digits!";
        if (cleaned.length() < 8 || cleaned.length() > 15) return "Phone must be 8-15 digits!";
        return null;
    }

    /** Validate IBAN (10-34 alphanumeric characters). */
    public static String validateIBAN(String iban) {
        if (iban == null || iban.trim().isEmpty()) return "IBAN is required!";
        String cleaned = iban.replaceAll("\\s", "").toUpperCase();
        if (cleaned.length() < 10 || cleaned.length() > 34) return "IBAN must be 10-34 characters!";
        if (!IBAN_PATTERN.matcher(cleaned).matches())
            return "Invalid IBAN format! (e.g. TN5914207207100707129648)";
        return null;
    }

    /** Validate SWIFT/BIC code (8-20 characters). */
    public static String validateSWIFT(String swift) {
        if (swift == null || swift.trim().isEmpty()) return null; // optional
        String cleaned = swift.trim().toUpperCase();
        if (cleaned.length() < 8 || cleaned.length() > 20) return "SWIFT must be 8-20 characters!";
        if (!cleaned.matches("^[A-Z0-9]+$")) return "SWIFT must contain only letters and numbers!";
        return null;
    }
    ...
    /** Clean and format IBAN with spaces every 4 characters. */
