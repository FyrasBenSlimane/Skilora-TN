...
public class PDFGenerator {
    ...
    // Style PDF (aligned with HTML/CSS :root)
    ...
    /**
     * Generate an employee financial report. The user picks the destination via a FileChooser.
     * If {@code customSummary} is non-null it replaces the auto-generated summary block.
     */
    public static File generateEmployeeReport(
            int employeeId, String employeeName,
            String contractInfo, String bankInfo,
            String bonusInfo, String payslipInfo,
            Stage ownerStage, String customSummary) {

        String safeName = (employeeName != null ? employeeName : "Employe")
                .replaceAll("[^a-zA-Z0-9_-]", "_");
        String defaultName = "Rapport_Financier_" + safeName + "_" + LocalDate.now() + ".pdf";

        FileChooser fileChooser = createFileChooser(
                "Enregistrer le rapport financier", defaultName,
                "Rapports PDF (*.pdf)", "*.pdf",
        ...
        try {
            if (isPdf(file)) {
                try {
                    generateEmployeeReportPDFFromHtml(file, employeeId, employeeName,
                            contractInfo, bankInfo, bonusInfo, payslipInfo, customSummary);
                } catch (Exception e) {
                    e.printStackTrace();
                    if (file.exists()) file.delete();
                    generateEmployeeReportPDF(file, employeeId, employeeName,
                            contractInfo, bankInfo, bonusInfo, payslipInfo, customSummary);
                }
            } else {
                generateEmployeeReportHTML(file, employeeId, employeeName,
                        contractInfo, bankInfo, bonusInfo, payslipInfo, customSummary);
            }
            return file;
        } catch (Exception e) {
    ...
    public static File generateEmployeeReport(
            ...
            String bonusInfo, String payslipInfo,
            Stage ownerStage) {
        return generateEmployeeReport(employeeId, employeeName,
                contractInfo, bankInfo, bonusInfo, payslipInfo, ownerStage, null);
    }

    /**
     * Generate a report to a pre-chosen file (no FileChooser — safe for background threads).
     */
    public static File generateToFile(
            File file, int employeeId, String employeeName,
            String contractInfo, String bankInfo,
            String bonusInfo, String payslipInfo, String customSummary) {
        if (file == null) return null;
        try {
            if (isPdf(file)) {
                try {
    ...
    /**
     * Generate a payslip PDF/HTML. The user picks the destination via a FileChooser.
     */
    public static File generatePayslipPDF(
            com.skilora.finance.entity.PayslipRow payslip,
            String employeeName, Stage ownerStage) {

        String safeName = (employeeName != null ? employeeName : "Employe")
                .replaceAll("[^a-zA-Z0-9_-]", "_");
        String defaultName = "Bulletin_" + safeName + "_"
                + payslip.getMonth() + "-" + payslip.getYear() + ".pdf";

        FileChooser fileChooser = createFileChooser(
                "Enregistrer le bulletin de paie", defaultName,
                "Rapports PDF (*.pdf)", "*.pdf",
                "Fichiers HTML (*.html)", "*.html");
        ...
        try {
            if (isPdf(file)) {
                generatePayslipPDFInternal(file, payslip, employeeName);
            } else {
    ...
    private static FileChooser createFileChooser(String title, String initialFileName,
            String filterDesc1, String ext1, String filterDesc2, String ext2) {
        ...
        fc.setInitialDirectory(documents.isDirectory() ? documents : userHome);
    ...
    public static File ensureExtension(File file, FileChooser.ExtensionFilter selected) {
    ...
    private static void addSectionCard(Document document, String icon, String sectionTitle,
            Element content) throws DocumentException {
        ...
        header.addCell(new Phrase(icon + "  ",
                FontFactory.getFont(FontFactory.HELVETICA, 14, PDF_TEXT)));
        header.addCell(new Phrase(sectionTitle,
                FontFactory.getFont(FontFactory.HELVETICA_BOLD, 14, PDF_TEAL)));
    ...
    // ─── Employee Report – OpenPDF fallback ─────────────────────────────────

    private static void generateEmployeeReportPDF(File file, int employeeId, String employeeName,
            String contractInfo, String bankInfo, String bonusInfo, String payslipInfo,
            String customSummary) throws DocumentException, IOException {
        ...
        if (hasContent(contractInfo)) {
            addSectionCard(document, "\uD83D\uDCCB", "Contrat(s)",
                    new Paragraph(htmlToReadableText(contractInfo), fontBody));
        }
        if (hasContent(bankInfo)) {
            addSectionCard(document, "\uD83C\uDFE6", "Compte(s) Bancaire(s)",
                    new Paragraph(htmlToReadableText(bankInfo), fontBody));
        }
        if (hasContent(bonusInfo)) {
            addSectionCard(document, "\u2B50", "Primes",
                    new Paragraph(htmlToReadableText(bonusInfo), fontBody));
        }
        if (hasContent(payslipInfo)) {
            addSectionCard(document, "\uD83D\uDCFE", "Bulletins de Paie Récents",
                    new Paragraph(htmlToReadableText(payslipInfo), fontBody));
        }

        document.add(new Paragraph(" ", fontBody));
        document.add(new Paragraph("— Document confidentiel — Skilora Gestion Finance —", fontFooter));
        Paragraph seal = new Paragraph("\u2726",
                FontFactory.getFont(FontFactory.HELVETICA, 10, PDF_TEAL));
    ...
    private static void generatePayslipPDFInternal(File file,
            com.skilora.finance.entity.PayslipRow p, String empName)
        ...
        document.add(new Paragraph(" ", fontValue));
        document.add(new Paragraph("Salaire brut : "
                + String.format(Locale.FRANCE, "%.2f", p.getGross()) + " "
                + (p.getCurrency() != null ? p.getCurrency() : "TND"), fontValue));
        document.add(new Paragraph("Net a payer : "
                + String.format(Locale.FRANCE, "%.2f", p.getNet()) + " "
                + (p.getCurrency() != null ? p.getCurrency() : "TND"), fontAmount));
        document.add(new Paragraph(" ", fontValue));
        document.add(new Paragraph(
                "— Document genere electroniquement — Skilora Gestion Finance —", fontFooter));
        document.close();
    }

    private static PdfPCell cellPdf(String text, boolean header) {
        Font f = header
                ? FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10, PDF_TEXT)
    ...
    private static String loadRapportTemplate() throws IOException {
        URL url = PDFGenerator.class.getResource("/com/skilora/view/finance/rapport_financier_template.html");
        if (url == null)
            url = PDFGenerator.class.getClassLoader()
                    .getResource("com/skilora/view/finance/rapport_financier_template.html");
    ...
    private static String buildReportSummaryPlainText(String employeeName,
            String contractInfo, String bankInfo, String bonusInfo, String payslipInfo) {
        String name = (employeeName != null && !employeeName.isBlank()) ? employeeName : "L'employé";
        StringBuilder sb = new StringBuilder();
        sb.append("Ce rapport présente la situation financière de ").append(name).append(". ");
        ...
        if (hasContract) sb.append("Un ou plusieurs contrats sont enregistrés. ");
        if (hasBank)     sb.append("Des comptes bancaires sont renseignés pour le virement du salaire. ");
        if (hasBonus)    sb.append("Des primes ont été versées et figurent dans l'historique. ");
        if (hasPayslip)  sb.append("Des bulletins de paie sont disponibles pour consultation. ");
        if (!hasContract && !hasBank && !hasBonus && !hasPayslip)
            sb.append("Aucune donnée détaillée n'est disponible pour le moment.");
        return sb.toString().trim();
    }

    private static String buildReportSummary(String employeeName,
            String contractInfo, String bankInfo, String bonusInfo, String payslipInfo) {
        String name = (employeeName != null && !employeeName.isBlank()) ? employeeName : "L'employé";
        StringBuilder summary = new StringBuilder();
        summary.append("Ce rapport présente la situation financière de ")
        ...
        if (hasContract) summary.append("Un ou plusieurs contrats sont enregistrés. ");
        if (hasBank)     summary.append("Des comptes bancaires sont renseignés pour le virement du salaire. ");
        if (hasBonus)    summary.append("Des primes ont été versées et figurent dans l'historique. ");
        if (hasPayslip)  summary.append("Des bulletins de paie sont disponibles pour consultation. ");
        if (!hasContract && !hasBank && !hasBonus && !hasPayslip)
        ...
        return "<div class=\"section\"><div class=\"section-header\">"
                ...
                + "<span class=\"sec-title\">R\u00e9sum\u00e9</span></div>"
                + "<p class=\"summary-text\">" + summary.toString().trim() + "</p></div>\n";
    }

    private static String buildFullReportHtml(String employeeName,
            String contractInfo, String bankInfo, String bonusInfo,
            String payslipInfo, String customSummary) throws IOException {

        String empName = escape(employeeName != null ? employeeName : "—");
        String dateStr = LocalDate.now().format(DATE_FORMAT);
        int year = LocalDate.now().getYear();

        // HTML entity icons for XML compat (openhtmltopdf)
        String iconSummary  = "&#128204;";
        String iconContract = "&#128203;";
        String iconBank     = "&#127974;";
        String iconBonus    = "&#11088;";
        ...
        String sectionSummary = (customSummary != null && !customSummary.isBlank())
                ? "<div class=\"section\"><div class=\"section-header\">"
                  + "<span class=\"sec-icon\">" + iconSummary + "</span>"
                  + "<span class=\"sec-title\">R\u00e9sum\u00e9</span></div>"
                  + "<p class=\"summary-text\">" + escape(customSummary) + "</p></div>\n"
        ...
        if (contractInfo != null && !contractInfo.isEmpty()) {
            sectionContracts = "<div class=\"section\"><div class=\"section-header\">"
                    + "<span class=\"sec-icon\">" + iconContract + "</span>"
                    + "<span class=\"sec-title\">Contrat(s)</span></div>\n"
                    + contractInfo + "</div>\n";
        }
        String sectionBank = "";
        if (bankInfo != null && !bankInfo.isEmpty()) {
            sectionBank = "<div class=\"section\"><div class=\"section-header\">"
                    + "<span class=\"sec-icon\">" + iconBank + "</span>"
                    + "<span class=\"sec-title\">Compte(s) Bancaire(s)</span></div>\n"
                    + bankInfo + "</div>\n";
        }
        String sectionBonus = "";
        if (bonusInfo != null && !bonusInfo.isEmpty()) {
            sectionBonus = "<div class=\"section\"><div class=\"section-header\">"
                    + "<span class=\"sec-icon\">" + iconBonus + "</span>"
                    + "<span class=\"sec-title\">Primes</span></div>\n"
                    + bonusInfo + "</div>\n";
        }
        String sectionPayslips = "";
        if (payslipInfo != null && !payslipInfo.isEmpty()) {
            sectionPayslips = "<div class=\"section\"><div class=\"section-header\">"
                    + "<span class=\"sec-icon\">" + iconPayslip + "</span>"
                    + "<span class=\"sec-title\">Bulletins de Paie R\u00e9cents</span></div>\n"
                    + payslipInfo + "</div>\n";
        }
        ...
        return buildRapportHtmlFallback(empName, dateStr, year,
                sectionSummary, sectionContracts, sectionBank, sectionBonus, sectionPayslips);
    }

    // ─── PDF via HTML→PDF (OpenHTMLtoPDF) ───────────────────────────────────

    private static void generateEmployeeReportPDFFromHtml(File file, int employeeId,
            String employeeName, String contractInfo, String bankInfo,
            String bonusInfo, String payslipInfo, String customSummary) throws IOException {

        String html = buildFullReportHtml(employeeName, contractInfo, bankInfo,
                bonusInfo, payslipInfo, customSummary);
    ...
    private static void generateEmployeeReportHTML(File file, int employeeId,
            String employeeName, String contractInfo, String bankInfo,
            String bonusInfo, String payslipInfo, String customSummary) throws IOException {
        String html = buildFullReportHtml(employeeName, contractInfo, bankInfo,
                bonusInfo, payslipInfo, customSummary);
    ...
    // ─── HTML Fallback (inline CSS) ─────────────────────────────────────────

    private static String buildRapportHtmlFallback(String empName, String dateStr, int year,
            String sectionSummary, String sectionContracts, String sectionBank,
            String sectionBonus, String sectionPayslips) {
        String template = "<!DOCTYPE html><html lang=\"fr\"><head><meta charset=\"UTF-8\"/><style>"
                ...
                + "<div class=\"header\"><h1>Rapport Financier</h1><p class=\"subtitle\">Dossier Employé • Confidentiel</p></div>"
                + "<div class=\"top-meta\"><strong>Employé :</strong> {{EMPLOYEE_NAME}} &nbsp;&nbsp;|&nbsp;&nbsp; <strong>Date d'émission :</strong> {{DATE_EMIS}}</div>"
                + "<table class=\"main-container\"><tr>"
                + "<td class=\"left-col\"><h3 class=\"section-title\">Résumé Profil</h3><div class=\"summary-box\">{{SECTION_SUMMARY}}</div>"
                + "<h3 class=\"section-title\">Coordonnées Bancaires</h3>{{SECTION_BANK}}"
                + "<h3 class=\"section-title\" style=\"margin-top: 40px;\">Bilan des Primes</h3>{{SECTION_BONUS}}</td>"
                + "<td class=\"right-col\"><h3 class=\"section-title\">Dossier &amp; Contrats</h3>{{SECTION_CONTRACTS}}"
                + "<h3 class=\"section-title\" style=\"margin-top: 40px;\">Historique de Paie</h3>{{SECTION_PAYSLIPS}}</td>"
                + "</tr></table>"
                + "<div class=\"footer\">© {{YEAR}} Skilora Gestion Finance — Document généré automatiquement</div>"
                + "</div></body></html>";
    ...
    private static void generatePayslipHTML(File file,
            com.skilora.finance.entity.PayslipRow p, String empName) throws IOException {

        StringBuilder html = new StringBuilder();
        html.append("<!DOCTYPE html>\n<html lang=\"fr\">\n<head>\n  <meta charset=\"UTF-8\">\n");
        html.append("  <title>Bulletin de paie — ").append(p.getMonth()).append("/")
                .append(p.getYear()).append("</title>\n");
        html.append("  <style>\n");
        ...
        html.append("      <div class=\"periode\"><h2>Période</h2><p>")
                .append(p.getMonth()).append(" / ").append(p.getYear()).append("</p></div>\n");
        html.append("    </div>\n");
        ...
        html.append("      <tr><td>Salaire de base</td><td class=\"montant\">")
                .append(formatNum(p.getBaseSalary())).append("</td></tr>\n");
        html.append("      <tr><td>Heures supplémentaires (").append(p.getOvertime())
                ...
                .append("</td></tr>\n");
        html.append("      <tr><td>Primes</td><td class=\"montant\">")
                .append(formatNum(p.getBonuses())).append("</td></tr>\n");
        ...
        html.append("      <div class=\"total-row\"><span>Total brut</span><strong>")
                .append(formatNum(p.getGross())).append(" ")
                .append(escape(p.getCurrency() != null ? p.getCurrency() : "TND"))
                .append("</strong></div>\n");
        html.append("      <div class=\"net\"><span>Net à payer</span><span>")
                .append(formatNum(p.getNet())).append(" ")
                .append(escape(p.getCurrency() != null ? p.getCurrency() : "TND"))
                .append("</span></div>\n");
        html.append("    </div>\n");
