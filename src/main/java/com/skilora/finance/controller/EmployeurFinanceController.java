...
import com.skilora.finance.service.TaxConfigurationService;
...
public class EmployeurFinanceController implements Initializable {
    ...
    @FXML private Label lbl_subtitle;
    @FXML private Label lbl_date;
    @FXML private Label lbl_userId;

    @FXML private Label kpi_net;
    @FXML private Label kpi_net_sub;
    @FXML private Label kpi_gross;
    @FXML private Label kpi_gross_sub;
    @FXML private Label kpi_bonuses;
    @FXML private Label kpi_bonuses_sub;
    ...
    @FXML private Label lbl_cont_name;
    @FXML private Label lbl_cont_position;
    @FXML private Label lbl_cont_type;
    @FXML private Label lbl_cont_salary;
    @FXML private Label lbl_cont_start;
    @FXML private Label lbl_cont_status;
    ...
    @FXML private javafx.scene.layout.HBox employeeCardContainer;
    @FXML private javafx.scene.layout.HBox teamStatsContainer;
    @FXML private TextField txt_search_employee;

    @FXML private javafx.scene.control.Label lblChatbotTitle;
    @FXML private javafx.scene.control.ScrollPane chatScrollPane;
    @FXML private VBox chatMessageContainer;
    @FXML private TextField chatQuestionField;

    private List<EmployeeSummaryRow> allEmployees = new ArrayList<>();

    @FXML private TableView<PayslipRow> tbl_payslips;
    @FXML private TableColumn<PayslipRow, String> col_ps_period;
    @FXML private TableColumn<PayslipRow, Double> col_ps_base;
    @FXML private TableColumn<PayslipRow, Double> col_ps_overtime;
    @FXML private TableColumn<PayslipRow, Double> col_ps_bonus;
    @FXML private TableColumn<PayslipRow, Double> col_ps_gross;
    @FXML private TableColumn<PayslipRow, Double> col_ps_cnss;
    @FXML private TableColumn<PayslipRow, Double> col_ps_irpp;
    @FXML private TableColumn<PayslipRow, Double> col_ps_net;
    @FXML private TableColumn<PayslipRow, String> col_ps_status;
    @FXML private Label lbl_payslip_count;

    @FXML private Label lbl_summary_net;
    @FXML private Label lbl_summary_cnss;
    @FXML private Label lbl_summary_irpp;
    @FXML private TextArea area_tax_detail;

    @FXML private PieChart pie_tax_breakdown;
    @FXML private Label lbl_tax_bracket;
    @FXML private Label lbl_tax_recommendation;
    @FXML private VBox vbx_tax_analysis;

    @FXML private TableView<BonusRow> tbl_bonuses;
    @FXML private TableColumn<BonusRow, Double> col_bon_amount;
    @FXML private TableColumn<BonusRow, String> col_bon_reason;
    @FXML private TableColumn<BonusRow, String> col_bon_date;
    @FXML private Label lbl_bonus_count;

    @FXML private TableView<BankAccountRow> tbl_banks;
    @FXML private TableColumn<BankAccountRow, String> col_bank_name;
    @FXML private TableColumn<BankAccountRow, String> col_bank_iban;
    @FXML private TableColumn<BankAccountRow, String> col_bank_swift;
    @FXML private TableColumn<BankAccountRow, String> col_bank_currency;
    @FXML private TableColumn<BankAccountRow, Boolean> col_bank_primary;
    @FXML private TableColumn<BankAccountRow, Boolean> col_bank_verified;
    @FXML private Label lbl_bank_count;

    @FXML private TableView<ContractRow> tbl_contracts;
    @FXML private TableColumn<ContractRow, String> col_cnt_company;
    @FXML private TableColumn<ContractRow, String> col_cnt_type;
    @FXML private TableColumn<ContractRow, String> col_cnt_position;
    @FXML private TableColumn<ContractRow, Double> col_cnt_salary;
    @FXML private TableColumn<ContractRow, String> col_cnt_start;
    @FXML private TableColumn<ContractRow, String> col_cnt_end;
    @FXML private TableColumn<ContractRow, String> col_cnt_status;
    @FXML private Label lbl_contract_count;

    private int employeeId = -1;
    private String employeeName = "Employe";
    ...
    private void setupSearch() {
        if (txt_search_employee != null) {
            txt_search_employee.textProperty().addListener((obs, oldVal, newVal) -> filterEmployeeCards(newVal));
        }
    }

    private void filterEmployeeCards(String filter) {
        if (employeeCardContainer == null) return;
    ...
    private void loadEmployeeCards() {
        ...
        try {
            ...
            for (EmployeeSummaryRow emp : allEmployees) {
                ...
                if (!"Verifié".equalsIgnoreCase(emp.getBankStatus())) bankIssues++;
            }
    ...
    private void updateTeamStats(int count, double gross, double net, int alerts) {
        if (teamStatsContainer == null) return;
    ...
    private HBox createStatMiniCard(String title, String value, String icon, String color) {
        ...
        box.setStyle("-fx-background-color: -fx-background; -fx-padding: 12; -fx-background-radius: 12; -fx-border-color: -fx-border; -fx-border-width: 1;");
        Label iconLbl = new Label(icon);
        iconLbl.setStyle("-fx-font-size: 20px; -fx-min-width: 40px; -fx-min-height: 40px; -fx-alignment: center; -fx-background-color: " + color + "22; -fx-text-fill: " + color + "; -fx-background-radius: 10;");
    ...
    private VBox createEmployeeCard(EmployeeSummaryRow emp) {
        ...
        if (emp.getUserId() == this.employeeId) card.setStyle(selectedStyle);
        else card.setStyle(baseStyle);
        Label avatar = new Label(getInitials(emp.getFullName()));
        avatar.setStyle("-fx-background-color: " + getAvatarColor(emp.getUserId()) + "; -fx-text-fill: white; -fx-font-weight: bold; -fx-font-size: 14px; -fx-min-width: 36px; -fx-min-height: 36px; -fx-max-width: 36px; -fx-max-height: 36px; -fx-background-radius: 18; -fx-alignment: center;");
        ...
        card.setOnMouseClicked(e -> setEmployeeId(emp.getUserId(), emp.getFullName()));
        card.setOnMouseEntered(e -> {
            if (emp.getUserId() != this.employeeId)
                card.setStyle(baseStyle + "-fx-background-color: -fx-control-inner-background; -fx-border-color: -fx-muted-foreground;");
        });
        card.setOnMouseExited(e -> {
            if (emp.getUserId() != this.employeeId) card.setStyle(baseStyle);
            else card.setStyle(selectedStyle);
        });
        return card;
    }
    ...
    private void loadAllData() {
        if (employeeId <= 0) {
            if (lbl_greeting != null) lbl_greeting.setText("Aucun utilisateur connecte.");
            return;
        }
        loadPayslips();
        loadContracts();
    ...
    private void loadPayslips() {
        try {
            ...
            list.sort(Comparator.comparingInt(PayslipRow::getYear).reversed().thenComparingInt(PayslipRow::getMonth).reversed());
            if (tbl_payslips != null) tbl_payslips.setItems(FXCollections.observableArrayList(list));
            if (lbl_payslip_count != null) lbl_payslip_count.setText(list.size() + " bulletin(s) de paie");
            if (!list.isEmpty()) {
    ...
    private void loadContracts() {
        try {
            ...
            if (tbl_contracts != null) tbl_contracts.setItems(FXCollections.observableArrayList(list));
            if (lbl_contract_count != null) lbl_contract_count.setText(list.size() + " contrat(s)");
            ...
            if (active != null) {
                ...
                if (lbl_cont_status != null) {
                    ...
                    lbl_cont_status.setStyle(ok
                            ? "-fx-text-fill:#22c55e;-fx-background-color:rgba(34,197,94,0.15);-fx-padding:3 10;-fx-background-radius:10;-fx-font-weight:bold;"
                            : "-fx-text-fill:#f59e0b;-fx-background-color:rgba(245,158,11,0.15);-fx-padding:3 10;-fx-background-radius:10;-fx-font-weight:bold;");
                }
            } else {
                set(lbl_cont_name, employeeName);
    ...
    private void loadBonuses() {
        try {
            ...
            if (tbl_bonuses != null) tbl_bonuses.setItems(FXCollections.observableArrayList(list));
            ...
            if (lbl_bonus_count != null) lbl_bonus_count.setText(list.size() + " prime(s)");
        } catch (Exception e) {
    ...
    private void loadBankAccounts() {
        try {
            ...
            if (tbl_banks != null) tbl_banks.setItems(FXCollections.observableArrayList(list));
            if (lbl_bank_count != null) lbl_bank_count.setText(list.size() + " compte(s) enregistre(s)");
        } catch (Exception e) {
    ...
    private void buildSalaryChart() {
        ...
        try {
            ...
            list.sort(Comparator.comparingInt(PayslipRow::getYear).thenComparingInt(PayslipRow::getMonth));
            List<PayslipRow> recent = list.size() > 12 ? list.subList(list.size() - 12, list.size()) : list;
    ...
    private void buildTaxDetail() {
        try {
            ...
            if (refSalary == 0 && !payslips.isEmpty()) refSalary = payslips.get(0).getBaseSalary();
            ...
            sb.append("╚══════════════════════════════════════════════════════╝\n\n");
            if (refSalary > 0) {
                Map<String, BigDecimal> bd = TaxConfigurationService.calculateCompleteSalary(BigDecimal.valueOf(refSalary));
                ...
                sb.append(" CNSS part patronale (16.57%): ").append(fmtR(cnssEmpl)).append(" TND [a la charge de l'employeur]\n");
                ...
                Map<String, String> recs = TaxConfigurationService.getOptimizationRecommendations(BigDecimal.valueOf(refSalary));
            ...
            if (!payslips.isEmpty()) {
                ...
                for (PayslipRow p : payslips) {
                    sb.append(String.format(" │ %-14s │ %12s │ %12s │ %12s │ %-10s │%n",
                            p.getPeriod(), fmtR(p.getGross()), fmtR(p.getCnss()), fmtR(p.getNet()), p.getStatus()));
                }
                sb.append(" └────────────────┴──────────────┴──────────────┴──────────────┴────────────┘\n");
            }
            if (area_tax_detail != null) area_tax_detail.setText(sb.toString());
        } catch (Exception e) {
            System.err.println("[Dashboard] buildTaxDetail: " + e.getMessage());
            if (area_tax_detail != null) area_tax_detail.setText("Erreur lors du calcul : " + e.getMessage());
        }
    }

    private void updatePieChart(double net, double cnss, double irpp) {
        if (pie_tax_breakdown == null) return;
        ...
        Platform.runLater(() -> {
            for (PieChart.Data data : pieData) {
                String color = "#7dd3fc";
                if (data.getName().contains("CNSS")) color = "#fbbf24";
                if (data.getName().contains("IRPP")) color = "#f87171";
                data.getNode().setStyle("-fx-pie-color: " + color + ";");
            }
        });
    }

    private <S, T> void bind(TableColumn<S, T> col, String prop) {
        if (col == null) return;
        col.setCellValueFactory(new PropertyValueFactory<>(prop));
    }

    private <S> void bindMoney(TableColumn<S, Double> col, String prop) {
        if (col == null) return;
        col.setCellValueFactory(new PropertyValueFactory<>(prop));
    ...
    private <S> void bindMoneyBold(TableColumn<S, Double> col, String prop, String color) {
        if (col == null) return;
        col.setCellValueFactory(new PropertyValueFactory<>(prop));
        col.setCellFactory(c -> new TableCell<>() {
            @Override
    ...
    private <S> void bindMoneyBoldBonus(TableColumn<S, Double> col, String prop, String color) {
        if (col == null) return;
        col.setCellValueFactory(new PropertyValueFactory<>(prop));
        col.setCellFactory(c -> new TableCell<>() {
            @Override
    ...
    private <S> void bindMoneyTND(TableColumn<S, Double> col, String prop) {
        if (col == null) return;
        col.setCellValueFactory(new PropertyValueFactory<>(prop));
        col.setCellFactory(c -> new TableCell<>() {
            @Override
    ...
    private <S> void bindStatusPayslip(TableColumn<S, String> col) {
        if (col == null) return;
    ...
    private <S> void bindStatusContract(TableColumn<S, String> col) {
        if (col == null) return;
        col.setCellValueFactory(new PropertyValueFactory<>("status"));
        col.setCellFactory(c -> new TableCell<>() {
            @Override
    ...
    private <S> void bindContractType(TableColumn<S, String> col) {
        if (col == null) return;
        ...
        col.setCellFactory(c -> new TableCell<>() {
            ...
            protected void updateItem(String t, boolean empty) {
                ...
                if (empty || t == null) { setText("--"); setStyle(""); return; }
    ...
    private <S> void bindIban(TableColumn<S, String> col) {
        if (col == null) return;
        ...
        col.setCellFactory(c -> new TableCell<>() {
            ...
            protected void updateItem(String iban, boolean empty) {
                ...
                if (empty || iban == null || iban.isBlank()) { setText("--"); return; }
                setText(iban.length() > 8 ? iban.substring(0, 4) + " **** " + iban.substring(iban.length() - 4) : iban);
            }
        });
    }

    private <S> void bindBool(TableColumn<S, Boolean> col, String prop, String trueLabel, String color) {
        if (col == null) return;
        col.setCellValueFactory(new PropertyValueFactory<>(prop));
        col.setCellFactory(c -> new TableCell<>() {
            @Override
            protected void updateItem(Boolean v, boolean empty) {
                ...
                lbl.setStyle(yes ? "-fx-text-fill:" + color + ";-fx-font-weight:bold;" : "-fx-text-fill:gray;");
                setGraphic(lbl);
                setText(null);
            }
        });
    ...
    private Label statusBadge(String status) {
        ...
        String color, bg;
        if (up.contains("PAID") || up.contains("PAYE") || up.contains("ACTIF") || up.contains("ACTIVE")) { color = "#22c55e"; bg = "rgba(34,197,94,0.15)"; }
        else if (up.contains("PENDING") || up.contains("ATTENTE")) { color = "#f59e0b"; bg = "rgba(245,158,11,0.15)"; }
        else if (up.contains("EXPIR") || up.contains("CANCEL") || up.contains("ANNUL")) { color = "#ef4444"; bg = "rgba(239,68,68,0.12)"; }
        else { color = "#6b7280"; bg = "rgba(107,114,128,0.12)"; }
        Label badge = new Label(up);
        badge.setStyle("-fx-text-fill:" + color + ";-fx-background-color:" + bg + ";-fx-padding:3 10;-fx-background-radius:10;-fx-font-size:11px;-fx-font-weight:bold;");
        return badge;
    }

    private void styleSeries(XYChart.Series<String, Number> series, String color) {
        try {
            if (series.getNode() != null) series.getNode().setStyle("-fx-stroke:" + color + ";-fx-stroke-width:2.5px;");
            for (XYChart.Data<String, Number> d : series.getData()) {
                if (d.getNode() != null) {
                    d.getNode().setStyle("-fx-background-color:" + color + ",white;-fx-background-insets:0,2;-fx-background-radius:5px;");
                    Tooltip.install(d.getNode(), new Tooltip(d.getXValue() + " : " + fmt(d.getYValue().doubleValue()) + " TND"));
                }
            }
    ...
    private void setBar(ProgressBar bar, Label pctLbl, double ratio) {
        ...
        if (bar != null) bar.setProgress(v);
        if (pctLbl != null) pctLbl.setText(String.format("%.1f%%", v * 100));
    }
    ...
    private String fmt(double v) {
        return String.format(java.util.Locale.US, "%,.2f", v).replace(",", " ").replace(".", ",");
    }
    ...
    private String monthFr(int m) {
        String[] names = { "Janvier", "Fevrier", "Mars", "Avril", "Mai", "Juin", "Juillet", "Aout", "Septembre", "Octobre", "Novembre", "Decembre" };
        return (m >= 1 && m <= 12) ? names[m - 1] : String.valueOf(m);
    }

    private String monthShort(int m) {
        String[] s = { "Jan", "Fev", "Mar", "Avr", "Mai", "Jun", "Jul", "Aou", "Sep", "Oct", "Nov", "Dec" };
    ...
    private FinanceChatbotService.FinanceChatContext buildChatContext() {
        ...
        for (EmployeeSummaryRow emp : allEmployees) {
            ...
            try {
                ...
                if (!ps.isEmpty()) {
                    ps.sort(Comparator.comparingInt(PayslipRow::getYear).reversed().thenComparingInt(PayslipRow::getMonth).reversed());
    ...
    @FXML private void handleRefreshAll() { loadAllData(); }
    @FXML private void handleRefreshPayslips() { loadPayslips(); buildSalaryChart(); buildTaxDetail(); }
    @FXML private void handleRefreshBonuses() { loadBonuses(); }
    @FXML private void handleRefreshBanks() { loadBankAccounts(); }
    @FXML private void handleRefreshContracts() { loadContracts(); }
}
