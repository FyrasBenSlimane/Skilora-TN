...
/**
 * FinanceChatbotService — Chatbot for the Finance (Ma Paie) module.
 *
 * If an OpenAI key is configured (openai.api.key in config/application.properties),
 * responses are generated by AI (GPT). Otherwise, rule-based FAQ responses are used.
 * Only answers questions related to payroll, payslips, CNSS, IRPP, contracts, bonuses, bank accounts.
...
public class FinanceChatbotService {
    ...
    private static volatile FinanceChatbotService instance;

    private final FinanceChatbotAIService aiService;

    /** Keywords indicating a finance/payroll-related question. */
    ...
    /** Pattern for off-topic detection. */
    ...
    private FinanceChatbotService() {
        this.aiService = FinanceChatbotAIService.getInstance();
    }

    public static FinanceChatbotService getInstance() {
        if (instance == null) {
            synchronized (FinanceChatbotService.class) {
                if (instance == null) {
                    instance = new FinanceChatbotService();
                }
            }
        }
        return instance;
    ...
    /**
     * Checks if the question is related to finance/payroll.
     */
    ...
    /**
     * Indicates whether the chatbot uses AI (OpenAI) to respond.
     */
    ...
    /**
     * Answers the user. If OpenAI key is configured, AI generates the response;
     * otherwise (or on API error), rule-based FAQ responses are used.
     *
     * @param question user question
     * @param context  optional context (employee name, last net, period, etc.)
     */
    public String answer(String question, FinanceChatContext context) {
        ...
        // If AI is configured, attempt AI response first
        ...
        if (!isFinanceRelated(question)) {
            return "Je ne réponds qu'aux questions sur la gestion finance et la paie "
                    + "(bulletins, salaire, CNSS, IRPP, primes, contrats, comptes bancaires). "
                    + "Pour tout autre sujet, merci de vous adresser aux ressources concernées.";
        }

        // Rule-based responses — used if AI disabled or as fallback
        ...
        if (matches(q, "c quoi le net", "c'est quoi le net", "qu'est ce que le net", "net a payer", "net à payer"))
            return "Le **salaire net à payer** est ce qui vous est effectivement versé après déduction de la part salariale CNSS (9,18 %) et de l'IRPP (impôt sur le revenu selon le barème).";
        ...
        if (matches(q, "cnss", "cotisation cnss", "part cnss"))
            return "La **CNSS** (Caisse Nationale de Sécurité Sociale) est une cotisation sociale. La part salariale est de 9,18 % sur le salaire brut. L'employeur verse en plus une part patronale (16,57 %).";

        if (matches(q, "irpp", "impot", "impôt", "impot sur le revenu"))
            return "L'**IRPP** (Impôt sur le Revenu des Personnes Physiques) est l'impôt sur le revenu en Tunisie, calculé selon un barème progressif.";

        if (matches(q, "prime", "primes", "bonus"))
            return "Les **primes** sont des montants ajoutés à votre rémunération (performance, ancienneté, etc.). Elles apparaissent sur le bulletin et sont prises en compte dans le brut.";

        if (matches(q, "bulletin", "fiche de paie", "bulletins"))
            return "Le **bulletin de paie** récapitule : salaire de base, heures sup., primes, déductions (CNSS, IRPP) et le net à payer. Consultez l'onglet « Historique des Paies ».";

        if (matches(q, "contrat", "type de contrat", "cdi", "cdd"))
            return "Votre **contrat** définit votre poste, le type (CDI, CDD…), le salaire et les dates. Les détails sont visibles dans la section « Détails du Contrat ».";

        if (matches(q, "compte bancaire", "iban", "virement", "versement"))
            return "Les **comptes bancaires** enregistrés sont utilisés pour le virement de votre salaire. L'IBAN apparaît masqué pour la sécurité.";

        if (matches(q, "heures sup", "heures supp", "overtime"))
            return "Les **heures supplémentaires** sont rémunérées en plus du salaire de base. Leur montant et nombre figurent sur chaque bulletin.";

        if (matches(q, "deduction", "déduction", "retenue"))
            return "Les **déductions** sur le bulletin sont principalement : la part salariale CNSS (9,18 %) et l'IRPP. Le net = brut − déductions.";

        if (matches(q, "mon net", "mon salaire", "combien je gagne", "dernier bulletin"))
            return buildPersonalizedNetAnswer(context);

        // Employee-specific salary query
        ...
        if (matches(q, "aide", "help", "aide moi", "comment"))
            return "Je peux vous expliquer : le salaire net et brut, la CNSS, l'IRPP, les primes, les bulletins de paie, les contrats et les comptes bancaires.";

        return "Votre question concerne bien la paie ou la finance. Pour une réponse plus précise, reformulez en mentionnant : bulletin, net, brut, CNSS, IRPP, prime ou compte bancaire.";
    }
    ...
    private String buildPersonalizedNetAnswer(FinanceChatContext context) {
        if (context != null && context.getLastPayslipNet() != null && context.getLastPayslipPeriod() != null) {
            return String.format(Locale.FRANCE,
                    "D'après vos données : **dernier bulletin** (%s), le **salaire net à payer** est de **%s TND**.",
                    context.getLastPayslipPeriod(),
                    context.getLastPayslipNet());
        }
        return "Le **salaire net** est indiqué sur chaque bulletin de paie. Consultez « Historique des Paies » pour voir le net de chaque période.";
    }

    private String buildEmployeeSalaryAnswer(String question, FinanceChatContext context) {
        if (context == null || context.getEmployeesByNormalizedName() == null
                || context.getEmployeesByNormalizedName().isEmpty())
            return null;
        ...
        for (Map.Entry<String, EmployeeSnapshot> e : context.getEmployeesByNormalizedName().entrySet()) {
            ...
            if (normalizedQ.contains(normalizedName)
                    || (fullName != null && fullName.length() > 0
                        && normalizedQ.contains(normalize(fullName.split("\\s+")[0])))) {
    ...
    /**
     * Optional context for personalizing responses (last payslip, employee name, etc.)
     * and for answering questions about other employees' salaries.
     */
    ...
    /** Employee data snapshot for personalized salary responses. */
