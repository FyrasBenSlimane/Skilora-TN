...
public class TLWindow extends BorderPane {
    ...
    public TLWindow(Stage stage, String title, Node content) {
        ...
        if (content != null) {
            // Ensure the main content (e.g. login view) starts just below the custom title bar
            // so that header videos/images never sit "behind" the top bar.
            BorderPane.setMargin(content, new Insets(8, 12, 12, 12));
            setCenter(content);
        } else {
            setCenter(content);
        }
