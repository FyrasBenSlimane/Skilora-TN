...
public class PostJobController implements Initializable {
    ...
    public void setEditingOffer(JobOffer offer) {
        ...
        if (offer != null) {
            ...
            // Map work type back to display type
            if (jobTypeSelect != null && offer.getWorkType() != null) {
                String wt = offer.getWorkType().toLowerCase().replace("_", "-");
                if (wt.length() > 0) {
                    wt = wt.substring(0, 1).toUpperCase() + wt.substring(1);
                    // Match with items if possible
                    for (String item : jobTypeSelect.getItems()) {
                        if (item.equalsIgnoreCase(wt)) {
                            jobTypeSelect.setValue(item);
                            break;
                        }
                    }
                }
            ...
            if (skillsList != null) {
                ...
                if (offer.getRequiredSkills() != null) {
                    for (String s : offer.getRequiredSkills()) {
                        addSkillBadge(s);
                    }
                }
            }
            ...
            if (publishBtn != null)
                publishBtn.setText(I18n.get("common.save"));
        }
    }
    ...
    private void updatePreview() {
        ...
        Label location = new Label("ðŸ“ "
                + (locationField.getText().isEmpty() ? I18n.get("postjob.no_location") : locationField.getText()));

        Label salary = new Label(
                "ðŸ’° " + (salaryField.getText().isEmpty() ? I18n.get("postjob.no_salary") : salaryField.getText()));
        ...
        new Thread(() -> {
            try {
                ...
                if (editingOffer != null) {
                    ...
                    boolean success = JobService.getInstance().updateJobOffer(offer);
                    id = success ? offer.getId() : -1;
                } else {
                    id = JobService.getInstance().createJobOffer(offer);
                }
