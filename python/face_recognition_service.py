...
class FaceRecognitionService:
    def __init__(self):
        ...
        self.encodings_dir = Path("biometric_data")
    ...
    def load_known_encodings(self):
        """Load all stored face encodings from disk"""
        encodings_file = self.encodings_dir / "encodings.json"
        if encodings_file.exists():
            try:
                with open(encodings_file, 'r') as f:
                    data = json.load(f)
                    for username, encoding_str in data.items():
    ...
    def save_encoding(self, username, encoding):
        ...
        data[username] = json.dumps(encoding.tolist())
